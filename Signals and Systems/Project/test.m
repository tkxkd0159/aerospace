...
for i=1:N
    for k=1:length(dcc)
    x_hat(i)=sum(2/N*(R(k)*cos(2*pi/N*i)-I(k)*sin(2*pi/N*i)));  
    end
end
...
future_n=N:N+round(avg_peri);

for i=N+1:N+round(peri)
    x(i)=x(i-N);
end

for i=N+1:N+round(avg_peri)
    x_hat(i)=x_hat(i-N);
end
...
ylabel('x[n] and the approximation to x[n]')
grid on
legend('x[n]','approximation to x[n]','Predict x[n]','Predict approximation to x[n]','Location','best')
...
for i=1:N+round(peri)
    all_x(i)=all_data(i)-all_data(1)+((all_data(1)-all_data(N))/(N-1))*(i-1);   % signal x[n]
end
fftstock2=fft(all_x);   
