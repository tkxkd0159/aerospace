...
plot(1:N,f_x_hatN(1:N))
 plot(future_n,all_x(future_n))
plot(future_n,f_x_hatN(future_n))

hold off
xlabel('n(day)')
ylabel('x[n] and the approximation to x[n]')
grid on
legend('x[n]','approximation to x[n]','Future x[n]','Future approximation to x[n]','Location','best')

title('x[n] and the Approximation to x[n] including predicted future data')
...
for k=1:4
R2(k)=real(fftstock2(dcc2(k)));
I2(k)=imag(fftstock2(dcc2(k)));
end

for i=1:N+34
    for k=1:4
    x_hat2(i,k)=2/(N+34)*(R2(k)*cos(2*pi/(N+34)*i)-I2(k)*sin(2*pi/(N+34)*i));    
    end
end
f_x_hat2=sum(x_hat2,2);
for i=1:N+34
    c_hat2(i)=f_x_hat2(i)+c(1)-((c(1)-all_data(N+34))/(N+34-1))*(i-1); 
end
%-------------------------------------------------------------%
%-----------------------------------------------------------------------%
